...
def run_beam_search(sess, model, vocab, batch, dqn = None, dqn_sess = None, dqn_graph = None):
  ...
  hyps = [Hypothesis(tokens=[vocab.convert_tokens_to_ids([data.START_DECODING])[0]],
                     log_probs=[0.0],
                     state=dec_in_state,
  ...
  while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
    ...
    latest_tokens = [t if t in range(len(vocab.vocab)) else vocab.convert_tokens_to_ids([data.UNKNOWN_TOKEN])[0] for t in latest_tokens] # change any in-article temporary OOV ids to [UNK] id, so that we can lookup word embeddings
    ...
    for h in sort_hyps(all_hyps): # in order of most likely h
      if h.latest_token == vocab.convert_tokens_to_ids([data.STOP_DECODING])[0]: # if stop token is reached...
